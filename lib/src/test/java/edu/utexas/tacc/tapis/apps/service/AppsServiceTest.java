...
//        bindFactory(AppsServiceJWTFactory.class).to(ServiceJWT.class);
...
//    }
//
//    App tmpApp = svc.getAppByName(authenticatedAdminUsr, apps[0].getName(), false, null);
...
//  //   name, appType, host, defaultAccessMethod, jobCanExec
//  @Test
...
//    int itemId = svc.createApp(authenticatedOwnerUsr, app0, scrubbedJson);
//    Assert.assertTrue(itemId > 0, "Invalid app id: " + itemId);
//    // Retrieve the app including the credential using the default access method defined for the app
//    // Use files service AuthenticatedUser since only certain services can retrieve the cred.
//    App tmpApp = svc.getAppByName(authenticatedFilesSvc, app0.getName(), true, null);
//    checkCommonSysAttrs(app0, tmpApp);
//  }
//
//  // Test updating a app
//  @Test
...
//    PatchApp patchApp = new PatchApp("description PATCHED", "hostPATCHED", false, "effUserPATCHED",
...
//    App tmpApp = svc.getAppByName(authenticatedOwnerUsr, app0.getName(), false, null);
////  App sysE = new App(-1, tenantName, "SsysE", "description E", AppType.LINUX, ownerUser, "hostE", true,
...
////  App sysE2 = new App(-1, tenantName, "SsysE", "description PATCHED", AppType.LINUX, ownerUser, "hostPATCHED", false,
...
//    app0.setHost("hostPATCHED");
//    app0.setEnabled(false);
//    app0.setEffectiveUserId("effUserPATCHED");
//    app0.setDefaultAccessMethod(prot2.getAccessMethod());
//    app0.setTransferMethods(prot2.getTransferMethods());
//    app0.setPort(prot2.getPort());
//    app0.setUseProxy(prot2.isUseProxy());
//    app0.setProxyHost(prot2.getProxyHost());
//    app0.setProxyPort(prot2.getProxyPort());
//    app0.setTags(tags2);
...
//    App tmpApp = svc.getAppByName(authenticatedTestUsr2, app0.getName(), false, null);
...
//    app0.setEffectiveUserId("${owner}");
//    app0.setBucketName("bucket8-${tenant}-${apiUserId}");
//    app0.setRootDir("/root8/${tenant}");
//    app0.setJobLocalWorkingDir("jobLocalWorkDir8/${owner}/${tenant}/${apiUserId}");
//    app0.setJobLocalArchiveDir("jobLocalArchDir8/${apiUserId}");
//    app0.setJobRemoteArchiveDir("jobRemoteArchDir8${owner}${tenant}${apiUserId}");
//    int itemId = svc.createApp(authenticatedOwnerUsr, app0, scrubbedJson);
//    Assert.assertTrue(itemId > 0, "Invalid app id: " + itemId);
//    App tmpApp = svc.getAppByName(authenticatedOwnerUsr, app0.getName(), false, null);
//    Assert.assertNotNull(tmpApp, "Failed to create item: " + app0.getName());
//    App.out.println("Found item: " + app0.getName());
//
//// sys8 = {tenantName, "Ssys8", "description 8", AppType.LINUX.name(), "${apiUserId}", "host8",
...
//    Assert.assertEquals(tmpApp.getHost(), app0.getHost());
//    Assert.assertEquals(tmpApp.getEffectiveUserId(), effectiveUserId);
//    Assert.assertEquals(tmpApp.getDefaultAccessMethod().name(), app0.getDefaultAccessMethod().name());
//    Assert.assertEquals(tmpApp.isEnabled(), app0.isEnabled());
//    Assert.assertEquals(tmpApp.getBucketName(), bucketName);
//    Assert.assertEquals(tmpApp.getRootDir(), rootDir);
//    Assert.assertEquals(tmpApp.getJobLocalWorkingDir(), jobLocalWorkingDir);
//    Assert.assertEquals(tmpApp.getJobLocalArchiveDir(), jobLocalArchiveDir);
//    Assert.assertEquals(tmpApp.getJobRemoteArchiveDir(), jobRemoteArchiveDir);
//    Assert.assertEquals(tmpApp.getPort(), app0.getPort());
//    Assert.assertEquals(tmpApp.isUseProxy(), app0.isUseProxy());
//    Assert.assertEquals(tmpApp.getProxyHost(), app0.getProxyHost());
//    Assert.assertEquals(tmpApp.getProxyPort(), app0.getProxyPort());
//    List<TransferMethod> tMethodsList = tmpApp.getTransferMethods();
//    Assert.assertNotNull(tMethodsList);
//    Assert.assertNotNull(app0.getTransferMethods());
//    for (TransferMethod txfrMethod : app0.getTransferMethods())
//    {
...
//      App.out.println("Found item: " + name);
//    }
...
//    for (App app : apps) {
//      App.out.println("Found item with id: " + app.getId() + " and name: " + app.getName());
//    }
//  }
//
...
//    App tmpApp2 = svc.getAppByName(authenticatedOwnerUsr, app0.getName(), false, null);
...
//  @Test(expectedExceptions = {IllegalStateException.class},  expectedExceptionsMessageRegExp = "^APPLIB_SYS_EXISTS.*")
...
//    // Now attempt to create again, should get IllegalStateException with msg APPLIB_SYS_EXISTS
...
//    App tmpApp = svc.getAppByName(authenticatedOwnerUsr, fakeAppName, false, null);
...
//  {
//    App app0 = apps[12];//17
//    PatchApp patchSys = new PatchApp("description PATCHED", "hostPATCHED", false, "effUserPATCHED",
//            prot2.getAccessMethod(), prot2.getTransferMethods(), prot2.getPort(), prot2.isUseProxy(), prot2.getProxyHost(),
//            prot2.getProxyPort(), cap2List, tags2, notes2);
//    patchSys.setName(app0.getName());
...
//    pass = false;
//    try { svc.getAppByName(authenticatedTestUsr0, app0.getName(), false, null); }
//    catch (NotAuthorizedException e)
//    {
//      Assert.assertTrue(e.getMessage().startsWith("HTTP 401 Unauthorized"));
//      pass = true;
...
//    pass = false;
//    try { svc.updateApp(authenticatedTestUsr0, patchSys, scrubbedJson); }
//    catch (NotAuthorizedException e)
//    {
//      Assert.assertTrue(e.getMessage().startsWith("HTTP 401 Unauthorized"));
//      pass = true;
...
//    Assert.assertTrue(pass);
//    pass = false;
//    try { svc.updateApp(authenticatedTestUsr1, patchSys, scrubbedJson); }
//    catch (NotAuthorizedException e)
//    {
//      Assert.assertTrue(e.getMessage().startsWith("HTTP 401 Unauthorized"));
//      pass = true;
...
//    Assert.assertTrue(pass);
//    pass = false;
//    try { svc.updateApp(authenticatedFilesSvc, patchSys, scrubbedJson); }
//    catch (NotAuthorizedException e)
//    {
//      Assert.assertTrue(e.getMessage().startsWith("HTTP 401 Unauthorized"));
//      pass = true;
...
//    boolean pass = true;
//    try { svc.getAppByName(authenticatedOwnerUsr, app0.getName(), false, null); }
//    catch (NotAuthorizedException e)
//    {
//      Assert.assertTrue(e.getMessage().startsWith("HTTP 401 Unauthorized"));
//      pass = false;
...
//    Assert.assertTrue(pass);
//    pass = true;
//    try { svc.getAppByName(authenticatedFilesSvc, app0.getName(), false, null); }
//    catch (NotAuthorizedException e)
//    {
//      Assert.assertTrue(e.getMessage().startsWith("HTTP 401 Unauthorized"));
//      pass = false;
...
//    Assert.assertTrue(pass);
//    pass = true;
//    try { svc.getAppByName(authenticatedTestUsr1, app0.getName(), false, null); }
//    catch (NotAuthorizedException e)
//    {
//      Assert.assertTrue(e.getMessage().startsWith("HTTP 401 Unauthorized"));
//      pass = false;
...
//    Assert.assertTrue(pass);
//    pass = true;
//    try { svc.getAppByName(authenticatedTestUsr2, app0.getName(), false, null); }
//    catch (NotAuthorizedException e)
//    {
//      Assert.assertTrue(e.getMessage().startsWith("HTTP 401 Unauthorized"));
//      pass = false;
...
//   */
//  private static void checkCommonSysAttrs(App app0, App tmpApp)
//  {
//    Assert.assertNotNull(tmpApp, "Failed to create item: " + app0.getName());
...
//    Assert.assertEquals(tmpApp.getHost(), app0.getHost());
//    Assert.assertEquals(tmpApp.getEffectiveUserId(), app0.getEffectiveUserId());
//    Assert.assertEquals(tmpApp.getDefaultAccessMethod().name(), app0.getDefaultAccessMethod().name());
//    Assert.assertEquals(tmpApp.isEnabled(), app0.isEnabled());
//    Assert.assertEquals(tmpApp.getBucketName(), app0.getBucketName());
//    Assert.assertEquals(tmpApp.getRootDir(), app0.getRootDir());
//    Assert.assertEquals(tmpApp.getJobLocalWorkingDir(), app0.getJobLocalWorkingDir());
//    Assert.assertEquals(tmpApp.getJobLocalArchiveDir(), app0.getJobLocalArchiveDir());
//    Assert.assertEquals(tmpApp.getJobRemoteArchiveApp(), app0.getJobRemoteArchiveApp());
//    Assert.assertEquals(tmpApp.getJobRemoteArchiveDir(), app0.getJobRemoteArchiveDir());
//    Assert.assertEquals(tmpApp.getPort(), app0.getPort());
//    Assert.assertEquals(tmpApp.isUseProxy(), app0.isUseProxy());
//    Assert.assertEquals(tmpApp.getProxyHost(), app0.getProxyHost());
//    Assert.assertEquals(tmpApp.getProxyPort(), app0.getProxyPort());
...
//    List<TransferMethod> tMethodsList = tmpApp.getTransferMethods();
//    Assert.assertNotNull(tMethodsList);
//    Assert.assertNotNull(app0.getTransferMethods(), "Orig TxfrMethods should not be null");
//    for (TransferMethod txfrMethod : app0.getTransferMethods())
//    {
//      Assert.assertTrue(tMethodsList.contains(txfrMethod), "List of transfer methods did not contain: " + txfrMethod.name());
//    }
