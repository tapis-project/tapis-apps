...
//    }
  ...
  private void checkAttrMisc(List<String> errMessages)
  {
    ...
    // If containerized and SINGULARITY and START runtime option is used it is an error.
    // Options should either be empty or SING_RUN.
    if (containerized && Runtime.SINGULARITY.equals(runtime))
    {
      // If options list contains START then reject.
      if ( runtimeOptions != null && runtimeOptions.contains(RuntimeOption.SINGULARITY_START))
      {
        errMessages.add(LibUtils.getMsg("APPLIB_CONTAINERIZED_SING_OPT"));
      }
    }
